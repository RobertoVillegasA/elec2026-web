"""
Servicio de Google Drive usando OAuth 2.0 (cuenta personal)
"""
...
class GoogleDriveService:
    ...
    def _initialize_service(self):
        ...
        try:
            ...
            if not os.path.exists(credentials_path):
                ...
                print(f"   Descarga el JSON OAuth y guardalo como: {GOOGLE_CREDENTIALS_FILE}")
                print("   Instrucciones en GOOGLE_DRIVE_PERSONAL_SETUP.md")
                return
            
            # Cargar credenciales
            self.creds = self._load_credentials(token_path, credentials_path)
            
            if not self.creds:
                print("ERROR: No se pudo cargar las credenciales")
                return
            ...
            # Verificar carpeta
            self.folder_id = self._verify_folder_access()
            
        except Exception as e:
            print(f"ERROR: Error inicializando Google Drive: {e}")
            self.service = None
    
    def _load_credentials(self, token_path: str, credentials_path: str) -> Optional[Credentials]:
        """Carga o adquiere credenciales"""
        creds = None
        ...
        if os.path.exists(token_path):
            try:
                ...
                print(f"WARNING: Error cargando token: {e}")
        
        # Si no hay token o no es valido, autenticar
    ...
    def _authenticate(self, credentials_path: str, token_path: str) -> Optional[Credentials]:
        ...
        try:
            ...
            # Usar puerto fijo 8085 para evitar problemas de redirect_uri
            ...
            print("3. Agrega este URI de redireccionamiento: http://localhost:8085/")
            print("4. Guarda los cambios")
            print("5. Ejecuta nuevamente: python test_drive_access.py")
            return None
    
    def _save_credentials(self, token_path: str, creds: Credentials):
    ...
    def _verify_folder_access(self) -> Optional[str]:
        """Verifica que la carpeta exista y sea accesible, o crea una nueva"""
        if not self.service:
            return None

        if not GOOGLE_DRIVE_FOLDER_ID:
            print("ERROR: Folder ID no configurado")
            return None

        try:
            print(f"Verificando acceso a carpeta: {GOOGLE_DRIVE_FOLDER_ID}")

            # Intentar obtener la carpeta directamente
            folder = self.service.files().get(
                fileId=GOOGLE_DRIVE_FOLDER_ID,
                fields='id, name, mimeType, webViewLink, owners, trashed',
            ).execute()

            if folder.get('trashed'):
                print("WARNING: La carpeta configurada esta en la papelera")
            ...
            if folder.get('mimeType') == 'application/vnd.google-apps.folder':
                ...
                print("WARNING: El ID no corresponde a una carpeta")
                print("Creando carpeta nueva...")
                return self._create_folder()

        except Exception as e:
            print(f"WARNING: Error accediendo a la carpeta configurada: {e}")
            print("Creando carpeta nueva...")
            return self._create_folder()
    
    def _create_folder(self) -> Optional[str]:
        """Crea una carpeta nueva en Google Drive"""
        try:
            print(f"Creando carpeta: {DRIVE_FOLDER_NAME}...")
            ...
            # Actualizar la configuracion con el nuevo ID
            print(f"\nActualiza GOOGLE_DRIVE_FOLDER_ID en google_drive_config.py:")
    ...
    def upload_image(self, image_data: bytes, filename: str, mime_type: str = 'image/jpeg') -> Optional[str]:
        ...
        try:
            ...
            # Hacer archivo publico (solo lectura)
...
def upload_acta_images(images: List[bytes], codigo_acta: str) -> List[str]:
    ...
    return drive_service.upload_images_batch(images, codigo_acta, 'a')


def upload_hoja_trabajo_images(images: List[bytes], codigo_acta: str) -> List[str]:
    """Sube imagenes de hoja de trabajo"""
    return drive_service.upload_images_batch(images, codigo_acta, 'h')
